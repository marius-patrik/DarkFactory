...
def test_every_rule_has_unique_stable_id_and_required_sections():
    ...
    files = _rule_files()
    assert files, "at least one canonical rule must exist"
    numbers = [int(name.split("-", 1)[0]) for name in files]
    assert numbers == list(range(1, len(files) + 1)), f"rule numbers must be contiguous: {numbers}"
    seen = set()
    for name in files:
