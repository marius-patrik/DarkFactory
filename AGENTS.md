...
- Index ordering is alphabetical by canonical term name. It uses outlined hierarchy `Rejstřík` level 1 → initial letter level 2 → each canonical term level 3, so every letter and term is visible in the depth-3 document outline. Aliases are deduplicated by stable term `id`.
...
- The toolbar path is `Home \\ work title \\ publication version \\ Final/Review/Split \\ PDF/Markdown/HTML \\ page switcher` for PDF. Mode and format are separate compact path selectors; PDF page navigation is the tail path segment. Switching publication version, mode, or format must preserve the other dimensions and must resolve directly to the generated artifact named by `variants.json`.
- Split mode uses the dedicated two-column/split-view icon. The page switcher belongs at the tail of the top path navigation; zoom controls remain in the bottom status bar. Zoom-out/in use minus/plus icons, theme/fullscreen controls also belong to the status bar, and fullscreen uses the Maximize/Minimize glyph pair. Zoom must support buttons, Ctrl/⌘+wheel/trackpad pinch, and two-touch pinch.
