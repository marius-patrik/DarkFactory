...
- Keywords remain a compact usage-driven list in front matter. Detailed term names/explanations belong only to the separate `Encyklopedie | Encyclopedia` generated by `render-encyclopedia()` in back matter immediately before `Seznam příloh | List of appendices`.
- Encyclopedia ordering is alphabetical by canonical term name. It uses outlined hierarchy `Encyklopedie` level 1 → initial letter level 2 → each used keyword level 3, so every letter and keyword is visible in the depth-3 document outline.
- Inline term links/★ markers target the stable `kw-<id>` encyclopedia entry; do not restore detailed entries under the keyword heading.
- Encyclopedia and appendices are outside the core-text word-count boundary.
