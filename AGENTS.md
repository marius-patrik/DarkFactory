...
- Keywords remain a compact usage-driven list. Detailed term names/explanations belong only to the separate `Encyklopedie | Encyclopedia` section generated by `render-encyclopedia()`.
...
- The toolbar path is `Home \\ work title \\ publication version \\ Final/Review`. Both publication version and Final/Review are compact path-segment selectors backed by the current viewer URLs; Final/Review must not live as a standalone toolbar icon.
