...
export function rewriteLinks(markdown: string, destPath: string): string {
  ...
  return markdown.replace(
    ...
    (match, target, anchor) => {
      ...
      if (!replacement) {
        const adrMatch = normalized.match(/^(?:\.agents\/notes\/)?adr\/(?<slug>[^/]+\.md)$/);
      ...
      if (!replacement) {
        const ruleMatch = normalized.match(/^(?:\.agents\/)?rules\/(?<slug>[^/]+\.md)$/);
...
export function generateManifestReference(manifest: any): string {
  const lines: string[] = [
    ...
    "Generated from `.github/darkfactory.json` at build time.",
    "",
...
export function stageDocs(repoRoot: string, stagingDir: string): void {
  ...
  const manifestPath = path.join(repoRoot, ".github", "darkfactory.json");
...
export async function buildDocs(options: BuildDocsOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (!options.skipProperdocs) {
      const proc = Bun.spawnSync(["properdocs", "build", "--strict"], {
        cwd: repoRoot,
        stdout: "inherit",
        stderr: "inherit",
      });
      ...
      if (proc.exitCode !== 0) {
        throw new Error(`properdocs build --strict failed with exit code ${proc.exitCode}`);
      }
    }
  } finally {
