...
const DEFAULTS: OAuthDependencies = {
	...
	now: Date.now,
	env: process.env,
	isHeadless: !(process.stdin.isTTY && process.stdout.isTTY),
	sleep,
};
function reference(
	ref: ValueReference | undefined,
	env: Readonly<Record<string, string | undefined>>,
	label: string,
): string | undefined {
...
function credential(
	...
	if (config.accountIdJwtClaim && !accountId)
		throw new Error("OAuth access token is missing the configured account id claim");
...
async function loopback(
	...
	state: string,
	signal: AbortSignal,
): Promise<LoopbackCallback | undefined> {
	let requested: URL;
	try {
		requested = new URL(config.redirectUri ?? "http://127.0.0.1:0/auth/callback");
	} catch {
		return undefined;
	}
	...
	server = createServer((request, response) => {
		...
		if (url.pathname !== requested.pathname) {
	...
	try {
		await new Promise<void>((resolve, reject) => {
			...
			server.listen(Number(requested.port || 80), requested.hostname, () => {
	...
	return {
		redirectUri: requested.toString(),
		result,
...
async function pkceLogin(
	...
	for (const [key, value] of Object.entries({
		client_id: reference(config.clientId, deps.env, "client id")!,
	...
	interaction.notify({
		...
		instructions: callback
			? "Complete sign-in; df is waiting on the local callback."
			: "Complete sign-in and paste the returned redirect URL.",
	});
	try {
		let code: string;
		if (callback) code = await callback.result;
			...
			const input = (
				await interaction.prompt({ type: "manual_code", message: "Authorization code or redirect URL" })
			).trim();
		...
		return await token(
			config,
			deps,
			{ grant_type: "authorization_code", code, state, redirect_uri: redirectUri, code_verifier: verifier },
			interaction.signal,
		);
	} finally {
...
async function deviceLogin(
	...
	const clientId = reference(config.clientId, deps.env, "client id")!;
	const response = await deps.fetch(config.deviceCodeEndpoint, {
		method: "POST",
		headers: { "content-type": "application/x-www-form-urlencoded" },
		body: new URLSearchParams({ client_id: clientId, scope: config.scopes.join(" ") }),
	...
	while (deps.now() < expires) {
		...
		const poll = await deps.fetch(config.tokenEndpoint, {
			method: "POST",
			headers: { "content-type": "application/x-www-form-urlencoded" },
 ...
 */
export function createConfiguredOAuth(
	config: OAuthAuthConfig,
	dependencies: Partial<OAuthDependencies> = {},
): OAuthAuth {
	const deps = { ...DEFAULTS, ...dependencies };
	return {
		name: config.loginLabel ?? "OAuth",
		loginLabel: config.loginLabel,
		isSubscription: config.isSubscription,
		login: (interaction) =>
			config.flow === "device_code" ? deviceLogin(config, deps, interaction) : pkceLogin(config, deps, interaction),
		refresh: (current, signal) =>
			token(config, deps, { grant_type: "refresh_token", refresh_token: current.refresh }, signal, current.refresh),
		async toAuth(current) {
			const extraHeaders = {
				...config.authHeaders,
				...(config.accountIdHeader && typeof current.accountId === "string"
					? { [config.accountIdHeader]: current.accountId }
					: {}),
			};
			return config.placement === "api_key"
				? { apiKey: current.access, headers: extraHeaders }
				: { headers: { Authorization: `Bearer ${current.access}`, ...extraHeaders } };
		},
	};
}
