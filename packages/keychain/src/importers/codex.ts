/**
 * OpenAI Codex / ChatGPT credential importer.
...
/** Parses ChatGPT OAuth triad from ~/.codex/auth.json. */
...
/** Options for finding Codex auth. */
export interface CodexFindOptions {
	homeReader: HomeReader;
}

/** Result of Codex auth scan. */
...
/** Finds Codex auth file and parses tokens or API key. */
...
/** Imports Codex ChatGPT login or metered API key. */
export async function importCodexAccount(
	...
	if (found.oauth) {
		...
		if (!oauth.refreshToken) {
			throw new Error("Codex login has no refresh token; it cannot self-heal and cannot be imported as an OAuth account");
		}
