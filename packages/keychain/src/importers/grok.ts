/**
 * Grok CLI OAuth credential importer.
...
/** Parsed Grok entry specification. */
...
/** Parses one `<issuer>::<client id>` entry. */
...
/** Options for finding Grok entries. */
export interface GrokFindOptions {
	homeReader: HomeReader;
}

/** Finds Grok entries in ~/.grok/auth.json. */
...
export async function importGrokAccount(
	...
	if (!entry.refreshToken) {
		throw new Error("Grok CLI login has no refresh token; it cannot self-heal and cannot be imported as an OAuth account");
	}
