...
test("DarkFactoryShell renders Dashboard at /", () => {
  const output = renderToString(createElement(MemoryRouter, { initialEntries: ["/"] }, createElement(DarkFactoryShell)));
  expect(output).toContain("Dashboard");
});

test("DarkFactoryShell renders System Status at /status", () => {
  const output = renderToString(createElement(MemoryRouter, { initialEntries: ["/status"] }, createElement(DarkFactoryShell)));
  expect(output).toContain("System Status");
});

test("DarkFactoryShell renders NotFound at /unknown", () => {
  const output = renderToString(createElement(MemoryRouter, { initialEntries: ["/unknown"] }, createElement(DarkFactoryShell)));
