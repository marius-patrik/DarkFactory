...
- **Dynamic GraphQL Quota Guard (`GRAPHQL_REMAINING` / `QUOTA_MINIMUM`)**: Project automation has no arbitrary per-run mutation cap. Immediately before every Projects v2 mutation it reads GitHub's live GraphQL `rateLimit.remaining` value using the project token, continues while it is above the 50-point safety reserve, and pauses cleanly at or below that reserve. This permits a run to use the available quota (up to GitHub's 5,000-point window) while retaining capacity for safe recovery.
- **Check-Before-Write Idempotency (`load_existing_items`)**: The project client queries and caches existing board items and their current statuses. If an issue or pull request already resides on the board with the desired status, no write mutation is issued, preserving GraphQL quota.
- **Scoped and Global Routing (`resolve_boards`)**: A repository's scoped board receives only that repository's items. During historical pipeline-wide reconciliation, items from other installed repositories are written to the Global board only, preventing their leakage into the pipeline repository's scoped board.
- **Quota-Safe Deferral**: When the live GraphQL reserve is reached during event handling or status reconciliation, remaining items are deferred to the next scheduled run. A malformed quota response is recorded as an infrastructure failure rather than treated as unlimited capacity.
