...
import { validateCaptureSchema, captureResult, CaptureError } from "../src/harness/result-capture.ts";
import { captureContext, readCapture } from "../src/harness/capture-request.ts";
import { z } from "zod";
import { createAssistantMessageEventStream } from "@earendil-works/pi-ai/utils/event-stream";
import type { Model, SimpleStreamOptions, AssistantMessage } from "@earendil-works/pi-ai";
...
function fakeCaptureStream(args: Record<string, unknown>, fail?: boolean): (model: Model<any>, context: any, options?: SimpleStreamOptions) => any {
...
describe("captureResult", () => {
  ...
  test("throws CaptureError when all fail", async () => {
    const candidates = [
      ...
      { dialect: "openai-completions" as any, model: mockModel("modelB"), stream: fakeCaptureStream({ name: "Bob", age: "bad" }) },
    ];
    await expect(captureResult({ answer: "Hello", schema: personSchema, candidates }))
...
describe("validateCaptureSchema", () => {
  const schema = z.object({ name: z.string(), age: z.number().int().nonnegative(), tags: z.array(z.string()).optional() });
