...
test("App JWT resolves installation and single-flights a long opaque token", async () => {
  ...
  const provider = new AppInstallationTokenProvider({ appId: "4861004", privateKey: pem, owner: "o", repo: "r", permissions: { issues: "write" } }, { fetch: mock.fetch, now: () => new Date("2029-01-01T00:00:00Z") });
  const [a, b] = await Promise.all([provider.getToken(), provider.getToken()]);
  expect(a).toBe(token); expect(b).toBe(token); expect(mock.calls).toHaveLength(2);
...
test("manifest app identity wins, with explicit PAT and GH_TOKEN fallback", async () => {
  const app = await appIdentityFromManifest({ app: { app_id: 4861004, slug: "darkfactory-pipeline", private_key_secret: "DARKFACTORY_APP_PRIVATE_KEY", permissions: { issues: "write" } } }, "o/r", name => name === "DARKFACTORY_APP_PRIVATE_KEY" ? "pem" : "");
