...
test("repository and environment secrets use sealed boxes and never send plaintext", async () => {
  ...
  const mock = scripted([json({ key_id: "kid", key: sodium.to_base64(key.publicKey, sodium.base64_variants.ORIGINAL) }), json(undefined, 204), json({ key_id: "ekid", key: sodium.to_base64(key.publicKey, sodium.base64_variants.ORIGINAL) }), json(undefined, 204)]);
