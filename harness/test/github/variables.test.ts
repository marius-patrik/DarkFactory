...
test("repository variable CRUD supports quota checkpoint names", async () => {
  const mock = scripted([json({ variables: [{ name: "DF_QUOTA_run-1", value: "{}", created_at: "x", updated_at: "x" }] }), json({ name: "DF_QUOTA_run-1", value: "{}", created_at: "x", updated_at: "x" }), json(undefined, 201), json(undefined, 204), json(undefined, 204)]);
  ...
  await repo.createVariable("DARKFACTORY_QUOTA_PROVIDERS", "{}"); await repo.updateVariable("DF_QUOTA_run-1", "next"); await repo.deleteVariable("DF_QUOTA_run-1");
