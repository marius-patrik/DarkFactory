...
async function sourceFiles(directory: string): Promise<string[]> {
	...
	for (const entry of await readdir(absolute, { withFileTypes: true })) {
		...
		else if (/\.ts$/u.test(entry.name)) files.push(relative);
	}
	return files;
}

describe("hard .df transition in the final TypeScript system", () => {
	test("shipped TypeScript/runtime/docs sources contain no legacy repo/config aliases or .df directories", async () => {
		const files = [
	...
	test("df-owned runtime state cannot regress to JSON/JSONL or non-df lock/temp aliases", async () => {
		const contracts: Record<string, string[]> = {
			"packages/keychain/src/credentials.ts": ["credentials.json", "vault.key", ".lock`", ".tmp`"],
