...
describe("runFailoverTurn", () => {
	test("fails over a quota response and preserves cross-provider tool/thinking history", async () => {
		...
		first.setResponses([
			fauxAssistantMessage([], {
				stopReason: "error",
				errorMessage: "429 insufficient_quota; reset in one hour token=hidden",
			}),
		]);
		second.setResponses([
			(context, options) => {
				...
				expect(
					historical.content.some((block) => block.type === "thinking" && block.thinking === "inspect state"),
				).toBe(true);
		...
		const context: Context = {
			messages: [
				...
				{
					...fauxAssistantMessage(
						[fauxThinking("inspect state"), fauxToolCall("lookup", { key: "value" }, { id: "call-1" })],
		...
		const result = await runFailoverTurn({
			...
			headersFor: (candidate): Record<string, string> =>
				candidate.provider === "faux-two" ? { "x-account-project": "test-project" } : {},
		...
		expect(result.steps.map((step) => step.errorMessage)).toEqual([
			"429 insufficient_quota; reset in one hour token=[REDACTED]",
			null,
		]);
