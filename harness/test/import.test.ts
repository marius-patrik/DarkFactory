...
describe("import claude", () => {
	test("imports the file login with refresh token, plan and organisation", async () => {
		...
		reader.write(".claude/.credentials.json", JSON.stringify({
			claudeAiOauth: {
		...
		expect(account?.slots.oauth).toEqual({ type: "oauth", access: "claude-access", refresh: "claude-refresh", expires: 1_900_000_000_000 });
	...
	test("falls back to a macOS keychain item when there is no credentials file", async () => {
		...
		const keyring: ClaudeKeyring = {
			async listServices() { return [service, "Some Other App", `${CLAUDE_CREDENTIALS_SERVICE_PREFIX}-def456`]; },
			async read(name) { return name === service ? JSON.stringify({ claudeAiOauth: { accessToken: "kc-access", refreshToken: "kc-refresh", expiresAt: 1_900_000_000_000 } }) : undefined; },
		};
		const store = new FileCredentialStore(home);
		...
		expect(account?.slots.oauth).toEqual({ type: "oauth", access: "kc-access", refresh: "kc-refresh", expires: 1_900_000_000_000 });
	...
	test("rejects a login without a refresh token (cannot self-heal)", async () => {
		const home = await temporaryHome();
		const reader = sandbox();
		reader.write(".claude/.credentials.json", JSON.stringify({ claudeAiOauth: { accessToken: "claude-access", expiresAt: 1_900_000_000_000 } }));
		const store = new FileCredentialStore(home);
		await expect(importClaudeAccount(store, "work", { home, homeReader: reader, keyring: emptyKeyring() }, "anthropic"))
	...
	test("rejects malformed JSON and an absent login without writing anything", async () => {
		const home = await temporaryHome();
		const store = new FileCredentialStore(home);
		await expect(importClaudeAccount(store, "work", { home, homeReader: sandbox(), keyring: emptyKeyring() }, "anthropic"))
...
describe("import codex", () => {
	const codexFixture = {
		tokens: {
			access_token: jwt({ exp: 2_000_000_000, "https://api.openai.com/auth": { chatgpt_plan_type: "pro", chatgpt_account_id: "acct-123" } }),
	...
	test("rejects missing refresh token / missing expiry / absent login", async () => {
		...
		await expect(importCodexAccount(store, "work", refresh, "openai-codex", "openai")).rejects.toThrow(/no refresh token/);

		const noExp = sandbox();
		noExp.write(".codex/auth.json", JSON.stringify({ tokens: { access_token: jwt({ "https://api.openai.com/auth": {} }), refresh_token: "r" } }));
		await expect(importCodexAccount(store, "work", noExp, "openai-codex", "openai")).rejects.toThrow(/no valid exp/);

		await expect(importCodexAccount(store, "work", sandbox(), "openai-codex", "openai")).rejects.toThrow(/No Codex login/);
		expect(await store.listAccounts()).toHaveLength(0);
	});
	...
	test("rejects malformed auth.json", async () => {
		...
		const store = new FileCredentialStore(home);
		await expect(importCodexAccount(store, "work", reader, "openai-codex", "openai")).rejects.toThrow(/Invalid JSON.*codex/);
	});
});
...
describe("import grok", () => {
	...
	test("imports the first entry of ~/.grok/auth.json keyed <issuer>::<client id>", async () => {
		const home = await temporaryHome();
		const reader = sandbox();
		reader.write(".grok/auth.json", JSON.stringify({ "https://issuer.example::client-a": entry, "https://issuer.example::client-b": { ...entry, key: "second" } }));
		const store = new FileCredentialStore(home);
		await importGrokAccount(store, "main", reader, "grok-sub");
		const account = await store.readAccount("grok-sub:main");
		expect(account?.slots.oauth).toEqual({ type: "oauth", access: entry.key, refresh: "grok-refresh", expires: Date.parse("2026-01-01T00:00:00Z") });
		expect(account?.metadata).toMatchObject({ account: "dev@example.com", issuer: "https://issuer.example", source: "grok-auth-json" });
	});
...
describe("import kimi and antigravity", () => {
	test("imports Kimi Code's rotating OAuth file and normalizes seconds", async () => {
		const home = await temporaryHome();
		const reader = sandbox();
		reader.write(".kimi-code/credentials/kimi-code.json", JSON.stringify({ access_token: "kimi-access", refresh_token: "kimi-refresh", expires_at: 2_000_000_000, scope: "openid" }));
		const store = new FileCredentialStore(home);
		await importKimiAccount(store, "main", reader, "kimi-coding", ".kimi-code/credentials/kimi-code.json");
		expect(await store.getSlot("kimi-coding", "main", "oauth")).toEqual({ type: "oauth", access: "kimi-access", refresh: "kimi-refresh", expires: 2_000_000_000_000 });
	...
	test("fills the bare Antigravity project slot from loadCodeAssist", async () => {
		const home = await temporaryHome();
		const store = new FileCredentialStore(home);
		const raw = JSON.stringify({ token: { access_token: "agy-access", refresh_token: "agy-refresh", expiry: "2030-01-01T00:00:00Z" } });
		await importAntigravityAccount(store, "main", { read: async () => raw }, "google-antigravity", "gemini", "antigravity", async (input, init) => {
	...
	test("rejects malformed Kimi and project discovery responses without an account", async () => {
		...
		await expect(importKimiAccount(store, "main", reader, "kimi-coding", ".kimi-code/credentials/kimi-code.json")).rejects.toThrow(/incomplete/);
		const raw = JSON.stringify({ token: { access_token: "agy-access", refresh_token: "agy-refresh", expiry: "2030-01-01T00:00:00Z" } });
		await expect(importAntigravityAccount(store, "main", { read: async () => raw }, "google-antigravity", "gemini", "antigravity", async () => Response.json({}))).rejects.toThrow(/cloudaicompanionProject/);
		expect(await store.listAccounts()).toHaveLength(0);
	});
});
