...
test("runGit passes arguments without a shell", () => {
	...
	if (!repo) throw new Error("Expected a temp repository");
	temp = repo;
	const hostile = "feature/$(echo pwned);x";
	expect(() => runGit(repo.repo, ["check-ref-format", "--branch", hostile])).toThrow(GitError);
	expect(runGit(repo.repo, ["log", "--format=%s", "-1"])).toBe("init");
});
