...
describe("Unix df wrapper", () => {
	test("uses DF_BIN for DarkFactory subcommands and a later PATH df for flags and non-interactive bare calls", async () => {
		...
		await Bun.write(join(wrapperDir, "df"), Bun.file(join(process.cwd(), "harness", "scripts", "df-wrapper.sh")));
		...
		const shellPath = (path: string) => process.platform === "win32" ? path.replace(/^([A-Za-z]):/u, (_match, drive: string) => `/${drive.toLowerCase()}`).replaceAll("\\", "/") : path;
		const env = { ...process.env, PATH: `${shellPath(wrapperDir)}:${shellPath(systemDir)}`, DF_BIN: shellPath(join(root, "df-bin")) };
		const invoke = async (args: string[]) => {
			const child = Bun.spawn([Bun.which("sh")!, shellPath(join(wrapperDir, "df")), ...args], { env, stdin: "pipe", stdout: "pipe", stderr: "pipe" });
			child.stdin.end(); return { out: await new Response(child.stdout).text(), exit: await child.exited };
		};
