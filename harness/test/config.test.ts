...
describe("local configuration and credential sources", () => {
	...
	test("rejects missing difficulty tier keys", async () => {
		await expect(
			loadDfConfig("C:/fixture", async () =>
				JSON.stringify({ router: { policies: [], difficultyTiers: { easy: "light", medium: "standard" } } }),
			),
		).rejects.toThrow("hard");
		await expect(
			loadDfConfig("C:/fixture", async () =>
				JSON.stringify({ router: { policies: [], difficultyTiers: { easy: "light", medium: "standard", hard: "" } } }),
			),
		).rejects.toThrow("hard");
	});
	...
	test("rejects invalid tier configuration", async () => {
		await expect(
			loadDfConfig("C:/fixture", async () =>
				JSON.stringify({ router: { policies: [], capabilityTiers: "not-an-array" } }),
			),
		).rejects.toThrow("must be an array");
		await expect(
			loadDfConfig("C:/fixture", async () =>
				JSON.stringify({ router: { policies: [], capabilityTiers: [{ id: "light", match: "*-mini*" }] } }),
			),
		).rejects.toThrow("must be an array of valid strings");
		await expect(
			loadDfConfig("C:/fixture", async () =>
				JSON.stringify({ router: { policies: [], capabilityTiers: [{ id: "", match: ["*-mini*"] }] } }),
			),
		).rejects.toThrow("id must be a non-empty string");
		await expect(
			loadDfConfig("C:/fixture", async () =>
				JSON.stringify({ router: { policies: [], capabilityTiers: [{ id: "light", match: [""] }] } }),
			),
		).rejects.toThrow("must be an array of valid strings");
		await expect(
			loadDfConfig("C:/fixture", async () => JSON.stringify({ router: { policies: [], defaultTier: "" } })),
