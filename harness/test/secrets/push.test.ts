...
describe("push sealed-box payload", () => {
	test("push encrypts value with sealed box and never sends plaintext, respects --only and --dry-run", async () => {
		...
		const vault: Vault = {
			version: 1,
			entries: [
				{ name: "GEMINI_API_KEY", value: "secret-123", scope: "actions", created: { by: "h", at: new Date().toISOString() }, updated: { by: "h", at: new Date().toISOString() } },
				{ name: "OTHER_KEY", value: "other-456", scope: "actions", created: { by: "h", at: new Date().toISOString() }, updated: { by: "h", at: new Date().toISOString() } },
			],
		};
		...
		expect(sodium.to_string(sodium.crypto_box_seal_open(encrypted, keypair.publicKey, keypair.privateKey))).toBe("secret-123");
		...
		const onlyResults = await pushSecrets({ vault, pushMap, repoSlug: "owner/repo", repository: repoOnly, only: ["GEMINI_API_KEY"] });
		...
		const dryClient = new GitHubClient({
			token: "t",
			fetch: async () => { fetchCalled = true; return json({}, 200); },
		});
		...
		const emptyResults = await pushSecrets({ vault, pushMap: pushMapOther, repoSlug: "owner/repo", repository: repoEmpty });
