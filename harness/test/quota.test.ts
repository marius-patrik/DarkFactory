...
describe("classifyFailure", () => {
	test.each([
		[
			"quota text",
			{ message: fauxAssistantMessage([], { stopReason: "error", errorMessage: "insufficient_quota" }) },
			"quota_exhausted",
		],
		...
		[
			"402 named in an SDK message without a status",
		...
		[
			"Cline daily cap with the status first",
			{
				message: fauxAssistantMessage([], {
					stopReason: "error",
					errorMessage:
						'429: {"code":"INFERENCE_CAP_ERROR","message":"Error 429: Daily free limit reached on model poolside/laguna-s-2.1:free. Try again in 22h 16m"}',
				}),
			},
			"quota_exhausted",
		],
		[
			"DeepInfra balance with the status first",
			{
				message: fauxAssistantMessage([], {
					stopReason: "error",
					errorMessage: '402: {"message":"You need positive balance to do inference. Please add balance manually or"}',
				}),
			},
			"quota_exhausted",
		],
		[
			"Hugging Face monthly credits",
			{
				message: fauxAssistantMessage([], {
					stopReason: "error",
					errorMessage: '402 "You have depleted your monthly included credits. Purchase pre-paid credits to continue"',
				}),
			},
			"quota_exhausted",
		],
		[
			"Ollama Cloud subscription model",
			{
				message: fauxAssistantMessage([], {
					stopReason: "error",
					errorMessage: '402: {"message":"this model requires a subscription or usage credits, upgrade for access"}',
				}),
			},
			"quota_exhausted",
		],
		[
			"status-first 429 without quota wording",
			{ message: fauxAssistantMessage([], { stopReason: "error", errorMessage: '429: {"message":"slow down"}' }) },
			"rate_limited",
		],
	...
	test.each([
		...
		expect(
			classifyFailure({ message: fauxAssistantMessage([], { stopReason: "error", errorMessage: wording }) }).kind,
		).toBe("quota_exhausted");
	});

	test("extracts Retry-After and reset epoch hints", () => {
		const now = 1_700_000_000_000;
		expect(classifyFailure({ error: providerError("429", 429, { "retry-after": "12" }), now }).resetAt).toBe(
			now + 12_000,
		);
		expect(
			classifyFailure({ error: providerError("429", 429, { "x-ratelimit-reset": "1700000090" }), now }).resetAt,
		).toBe(1_700_000_090_000);
	});
	...
	test("takes the reset a provider only states in prose", () => {
		const now = 1_700_000_000_000;
		const errorMessage =
			'429: {"code":"INFERENCE_CAP_ERROR","message":"Error 429: Daily free limit reached on model x. Try again in 22h 16m"}';
	...
	describe("AI Studio daily free-tier regression triplet", () => {
		...
		test("success: daily QuotaFailure is quota_exhausted and uses RetryInfo", () => {
			const body = JSON.stringify({
				error: { status: "RESOURCE_EXHAUSTED", details: [{ violations: [{ quotaId }] }, { retryDelay: "123s" }] },
			});
			expect(
				classifyFailure({ error: providerError(body, 429), now }, { rules, model: "gemini-3.8-flash" }),
			).toMatchObject({ kind: "quota_exhausted", resetAt: now + 123_000, pool: "google-free-daily:gemini-3.8-flash" });
		});

		test("edge-input: missing RetryInfo resets at next Pacific midnight", () => {
			const body = JSON.stringify({
				error: { status: "RESOURCE_EXHAUSTED", details: [{ violations: [{ quotaId }] }] },
			});
			expect(
				classifyFailure({ error: providerError(body, 429), now }, { rules, model: "gemini-3.8-flash" }).resetAt,
			).toBe(nextPacificMidnight(now));
		});

		test("failure: unrelated 429 remains rate_limited", () => {
			expect(
				classifyFailure(
					{ error: providerError('{"error":{"status":"RESOURCE_EXHAUSTED","message":"burst"}}', 429), now },
					{ rules, model: "gemini-3.8-flash" },
				).kind,
			).toBe("rate_limited");
		});
		...
		test("recorded-shape: extracts RetryInfo from a formatted 429 QuotaFailure body", () => {
			...
			expect(
				classifyFailure(
					{
						message: fauxAssistantMessage([], { stopReason: "error", errorMessage: formatted }),
						response: { status: 429, headers: {} },
						now,
					},
					{ rules, model: "gemini-3-flash-preview" },
				),
			).toMatchObject({
