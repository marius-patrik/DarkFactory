...
const provider = {
	...
	models: { static: [{ id: "text-model", name: "Text Model" }, { id: "missing-model", name: "Missing Model" }] },
...
describe("df models", () => {
	...
	test("--stale lists declared ids missing from the live list", async () => {
		const home = await mkdtemp(join(tmpdir(), "df-cli-models-"));
		temporary.push(home);
		await writeFile(join(home, "providers.json"), JSON.stringify({ version: 1, providers: [{ ...provider, models: { ...provider.models, list: { path: "/models", itemsPath: "data", idPath: "id" } } }] }), "utf8");
		await mkdir(join(home, "models"));
		await writeFile(join(home, "models", `${provider.id}.json`), JSON.stringify({ version: 1, provider: provider.id, fetchedAt: Date.now(), models: [{ id: "text-model", name: "Text Model" }] }), "utf8");
		const child = Bun.spawn([process.execPath, "run", "src/cli.ts", "models", "--provider", provider.id, "--stale"], {
			cwd: process.cwd(),
			env: { DF_HOME: home, DF_OFFLINE: "1", PATH: process.env.PATH ?? "", SYSTEMROOT: process.env.SYSTEMROOT ?? "C:\\Windows" },
			stdout: "pipe",
			stderr: "pipe",
		});
