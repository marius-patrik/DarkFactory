...
export class LimitLedger {
	...
	private async readExisting(): Promise<LedgerFile | undefined> {
		try {
			...
			if (
				...
				!Object.values(raw.entries).every(valid)
			)
				throw new Error("Invalid limits file");
	...
	async list(): Promise<LimitEntry[]> {
		return Object.values((await this.readRaw()).entries).sort((a, b) => a.resetAt - b.resetAt);
	}
	...
	record(entries: readonly LimitEntry[]): Promise<void> {
		...
		return this.modify((file) => {
			for (const entry of kept) file.entries[limitKey(entry)] = entry;
		});
	}
	...
	async forCandidate(candidate: Candidate, now = Date.now(), pools: readonly string[] = []): Promise<LimitEntry[]> {
		return (await this.list()).filter(
			(entry) =>
				...
				entry.resetAt > now &&
				(entry.model === candidate.model || (!!entry.pool && pools.includes(entry.pool))),
		);
	}
	...
	async blocking(
		...
		return (await this.forCandidate(candidate, now, pools)).filter(
			(entry) =>
	...
	async clear(selector: string): Promise<number> {
		let removed = 0;
		await this.modify((file) => {
			for (const [key, entry] of Object.entries(file.entries)) {
				const id = `${entry.provider}/${entry.model}@${entry.account}`;
				if (
					selector === "*" ||
					id === selector ||
					entry.provider === selector ||
					`${entry.provider}:${entry.account}` === selector
				) {
		...
		await appendFile(
			this.auditPath,
			`${JSON.stringify({ action: "clear", selector, removed, observedAt: Date.now() })}\n`,
	...
	clearAccount(provider: string, account: string): Promise<void> {
		return this.clear(`${provider}:${account}`).then(() => undefined);
	}
		...
		await withFileLock(this.lockPath, async () => {
			...
			for (const [key, entry] of Object.entries(file.entries)) {
				...
				if (entry.source === "default" && confirm && !(await confirm(entry))) {
					...
					entry.resetAt = now + Math.min(window * 2, 6 * 60 * 60_000);
					continue;
				}
