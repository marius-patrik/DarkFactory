...
export interface DetectedQualityExecution {
	packageId: string;
	kind: "test" | "lint" | "format_check" | "typecheck" | "docs_check";
	supported: boolean;
	command?: string;
	reason?: string;
	result?: VerifyResult;
...
/** Runs the supported detected test/lint/format/typecheck/docs_check actions without inventing fallback commands. */
export async function runDetectedQuality(
	...
	for (const entry of state.resolution.packages) {
		for (const kind of ["test", "lint", "format_check", "typecheck", "docs_check"] as const) {
