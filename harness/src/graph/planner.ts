...
function none(reason: string): PlanAction {
...
function actionFor(graph: WorkflowGraph, edge: GraphEdge, state: RunState, feedback?: string): PlanAction {
	...
	const alerts =
		edge.loop?.safety_budget !== undefined && (state.iterations?.[edge.from] ?? 0) >= edge.loop.safety_budget
			? [`${edge.from} exceeded safety budget ${edge.loop.safety_budget}`]
			: undefined;
	...
	return {
		...
		nodes: [target.id],
		...(feedback ? { feedback } : {}),
		...(edge.loop?.kind === "deviation_rework" ? { revert: "out_of_scope_commits" as const } : {}),
		...(alerts ? { alerts } : {}),
	};
}
...
function authorized(gate: GateNode, event: Extract<GraphEvent, { actor: unknown }>, state: RunState): boolean {
	return (
		!event.actor.is_bot &&
		(gate.author_associations.includes(event.actor.association as never) ||
			(gate.requester_can_approve === true && event.actor.login === state.requester))
	);
}
...
export function plan(graph: WorkflowGraph, event: GraphEvent, state: RunState): PlanAction {
	...
	if (event.type === "issues.opened" || event.type === "issues.labeled") {
		...
		if (root && event.type === "issues.labeled") {
			const ingress = graph.edges.find(
				(edge) => edge.from === root.id && "event" in edge.on && edge.on.event.split("|").includes(event.type),
			);
			if (ingress && "event" in ingress.on && ingress.on.filter.label && ingress.on.filter.label !== event.label)
				return none("event filter did not match");
		}
	...
	if (event.type === "node.completed") {
		...
		if (event.outcome === "quota_exhausted" && (current as AgentNode).quota_policy)
			return {
				type: "comment",
				node: current.id,
				status: "Blocked",
		...
		const edge = matchingEdge(
			graph,
			current.id,
			(item) => "node_outcome" in item.on && item.on.node_outcome === event.outcome,
			outputs,
		);
	...
	if (event.type === "checks.completed") {
		const edge = matchingEdge(
			graph,
			current.id,
			(item) => "checks" in item.on && item.on.checks === event.conclusion,
			state.outputs,
		);
	...
	if (event.type === "children.completed") {
		...
		const edge = matchingEdge(
			graph,
			current.id,
			(item) => "children" in item.on && item.on.children === event.outcome,
			state.outputs,
		);
		return edge ? actionFor(graph, edge, state) : none("no matching children edge");
	}
	if (event.type === "comment") {
		if (state.quota_blocked && RESUME.test(event.body ?? ""))
			return { type: "run", nodes: [current.id], resume_run_id: state.run_id };
		...
		if (!command)
			return state.hints.includes(current.id)
		...
		const edge = matchingEdge(
			graph,
			current.id,
			(item) => "gate_outcome" in item.on && item.on.gate_outcome === outcome,
			state.outputs,
		);
		return edge
			? actionFor(graph, edge, state, outcome === "rejected" ? event.body : undefined)
			: none("no matching gate edge");
	}
	if (event.type === "review") {
		...
		const edge = matchingEdge(
			graph,
			current.id,
			(item) => "gate_outcome" in item.on && item.on.gate_outcome === "approved",
			state.outputs,
		);
	...
	if (event.type === "schedule") {
		if (current.id === "resume-sweep") {
			...
			return checkpoint
				? { type: "run", nodes: [checkpoint.node], resume_run_id: checkpoint.run_id }
				: none("no eligible checkpoints");
		}
		if (
			current.kind === "gate" &&
			...
			state.blocked_since &&
			event.now &&
			Date.parse(event.now) - Date.parse(state.blocked_since) >= current.reminder_after_days * 86_400_000
		)
			return {
				type: "comment",
				node: current.id,
