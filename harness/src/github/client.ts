...
export class GitHubClient {
  ...
  async graphql<T>(query: string, variables: Record<string, unknown> = {}): Promise<T> {
    ...
    if (response.value.errors?.length || response.value.data === undefined) {
      throw new GitHubError({ kind: "graphql", message: redact(response.value.errors?.map(e => e.message ?? "unknown GraphQL error").join("; ") ?? "missing GraphQL data", [await this.#getToken()]), method: "POST", path: "/graphql", status: response.status, requestId: response.requestId });
    }
  ...
  async collectGraphQL<T>(query: string, variables: Record<string, unknown>, connection: (data: unknown) => GraphQLConnection<T>, limit = 10_000): Promise<T[]> {
    ...
    while (nodes.length < limit) {
      ...
      if (!page || !Array.isArray(page.nodes) || !page.pageInfo || typeof page.pageInfo.hasNextPage !== "boolean") throw this.#protocol("POST", "/graphql", "missing GraphQL connection pageInfo");
  ...
  async #request<T>(method: string, url: string, body: unknown, useEtag: boolean, accept = "application/vnd.github+json"): Promise<{ value: T; headers: Headers; status: number; requestId?: string }> {
    const token = await this.#getToken();
    const cacheKey = `${url}|${accept}`;
    const cached = method === "GET" && useEtag ? this.#cache.get(cacheKey) : undefined;
    for (let attempt = 0; ; attempt++) {
      const headers = new Headers({ Accept: accept, Authorization: `Bearer ${token}`, "X-GitHub-Api-Version": "2022-11-28", "User-Agent": this.#userAgent });
      if (body !== undefined) headers.set("Content-Type", "application/json");
      if (cached) headers.set("If-None-Match", cached.etag);
      let response: Response;
      try { response = await this.#transport.request(url, { method, headers, body: body === undefined ? undefined : JSON.stringify(body) }); }
      catch (cause) {
        if (attempt < this.#maxRetries) { await this.#sleep(this.#jitter(attempt)); continue; }
        throw new GitHubError({ kind: "transport", message: redact(cause instanceof Error ? cause.message : String(cause), [token]), method, path: safePath(url) });
      }
      const requestId = response.headers.get("x-github-request-id") ?? undefined;
      this.#readRestRate(response.headers);
      if (response.status === 304 && cached) return { value: cached.value as T, headers: response.headers, status: 304, requestId };
      ...
      if (attempt < this.#maxRetries && classification.retryable) { await this.#sleep(classification.delayMs ?? this.#jitter(attempt)); continue; }
      const remoteMessage = value && typeof value === "object" && "message" in value ? String((value as { message: unknown }).message) : `GitHub returned HTTP ${response.status}`;
      throw new GitHubError({ kind: classification.kind, message: redact(remoteMessage, [token]), method, path: safePath(url), status: response.status, requestId, retryAt: classification.retryAt });
    }
  }

  #jitter(attempt: number): number { return Math.floor(this.#random() * Math.min(30_000, 500 * 2 ** attempt)); }
  async #getToken(): Promise<string> { const token = typeof this.#token === "function" ? await this.#token() : this.#token; if (!token) throw new Error("GitHub token is required"); return token; }
  #protocol(method: string, path: string, message: string): GitHubError { return new GitHubError({ kind: "protocol", message, method, path: safePath(path) }); }
  #readRestRate(headers: Headers): void { const remaining = numberHeader(headers, "x-ratelimit-remaining"); const reset = numberHeader(headers, "x-ratelimit-reset"); if (remaining !== undefined && reset !== undefined) this.rateLimit = { resource: headers.get("x-ratelimit-resource") ?? "core", limit: numberHeader(headers, "x-ratelimit-limit"), remaining, resetAt: new Date(reset * 1000) }; }
  #readGraphQLRate(data: unknown): void { if (!data || typeof data !== "object" || !("rateLimit" in data)) return; const rate = (data as any).rateLimit; if (rate && typeof rate.remaining === "number" && typeof rate.resetAt === "string") this.rateLimit = { resource: "graphql", remaining: rate.remaining, resetAt: new Date(rate.resetAt), cost: typeof rate.cost === "number" ? rate.cost : undefined }; }
}

function safeJson(raw: string): unknown { try { return JSON.parse(raw); } catch { return raw; } }
function safePath(url: string): string { try { return new URL(url).pathname; } catch { return url; } }
function numberHeader(headers: Headers, name: string): number | undefined { const raw = headers.get(name); if (raw === null) return undefined; const value = Number(raw); return Number.isFinite(value) ? value : undefined; }
function parseNextLink(link: string | null): string | undefined { if (!link) return undefined; for (const part of link.split(",")) { const match = part.match(/^\s*<([^>]+)>\s*;\s*rel="?([^";]+)"?/); if (match?.[2] === "next") return match[1]; } return undefined; }
function classify(response: Response, value: unknown): { kind: GitHubErrorKind; retryable: boolean; delayMs?: number; retryAt?: Date } {
  const message = value && typeof value === "object" && "message" in value ? String((value as any).message).toLowerCase() : "";
  ...
  if ((response.status === 403 || response.status === 429) && (retryAfter !== undefined || message.includes("secondary rate limit") || message.includes("abuse detection"))) return { kind: "secondary-rate-limit", retryable: true, delayMs: retryAfter === undefined ? undefined : retryAfter * 1000 };
  if ((response.status === 403 || response.status === 429) && remaining === 0) { const reset = numberHeader(response.headers, "x-ratelimit-reset"); return { kind: "primary-rate-limit", retryable: false, retryAt: reset === undefined ? undefined : new Date(reset * 1000) }; }
