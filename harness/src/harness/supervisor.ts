...
export class ChainExhaustedError extends Error {
	...
	constructor(
		...
		reasons: readonly CandidateFailureReason[] = [],
		limits: readonly LimitEntry[] = [],
	) {
		...
		const quotaOnly =
			failures.length > 0 &&
			failures.every((kind) => kind === "quota_exhausted" || kind === "rate_limited" || kind === "transient");
		...
		super(
			authOnly
				? "All failover candidates failed authentication"
				: quotaOnly
					? "All failover candidates are exhausted or rate limited"
					: finalMessage
						? redactErrorMessage(finalMessage)
...
export class FailoverSupervisor {
	...
	get activeCandidate(): Candidate {
		return this.options.chain[this.activeIndex]!;
	}
	...
	private async recordOutcome(
		...
		await this.options.outcomeStore.record({
			candidate,
			kind: this.options.taskKind,
			success,
			...(failureKind ? { failureKind } : {}),
	...
	private async recordUsage(candidate: Candidate, usage: Usage | null | undefined, success: boolean): Promise<void> {
		...
		await this.options.quota.record({
			...candidate,
			timestamp: (this.options.now ?? Date.now)(),
			inputTokens: count("input") + count("cacheRead") + count("cacheWrite"),
			outputTokens: count("output"),
			success,
		});
	}

	private policy(candidate: Candidate) {
		return this.options.providerConfigs?.get(candidate.provider)?.limits;
	}
	...
	private pools(candidate: Candidate): string[] {
		return (this.policy(candidate)?.defaults ?? []).flatMap((entry) =>
	...
	private async usable(candidate: Candidate, now: number): Promise<{ usable: boolean; entries: LimitEntry[] }> {
		...
		for (const item of this.policy(candidate)?.defaults ?? []) {
			...
			entries.push({
				...
				...(item.pool ? { pool: item.pool.replace(":model", `:${candidate.model}`) } : {}),
				observedAt: now,
				resetAt: now + item.windowMs,
				source: "default",
				remaining: item.limit,
				limit: item.limit,
			});
		}
		const verdict = this.options.taskEstimate
			? assessCandidate(candidate, this.options.taskEstimate, entries, this.profile(candidate))
			: {
					eligible:
						(await this.options.ledger.blocking(candidate, now, this.policy(candidate)?.reserve, this.pools(candidate)))
							.length === 0,
				};
		// Only limits that actually block are cooldowns; an unexhausted default window must never be waited for.
		return {
			usable: verdict.eligible,
			entries: entries.filter((entry) => entry.remaining === undefined || entry.remaining <= 0),
		};
	}
	...
	private async nextCandidate(
			...
			if (!status.usable) {
				...
				this.emit({
					...
					reason:
						blocked?.type === "auth"
							? "auth"
							: blocked?.type === "overload"
								? "transient"
								: blocked?.type === "rate"
									? "rate_limited"
									: "quota_exhausted",
					...(blocked ? { resetAt: blocked.resetAt } : {}),
				});
				continue;
			}
			this.activeIndex = index;
		...
		for (const item of candidatesToConsider)
			readyAt.set(item.index, Math.max(readyAt.get(item.index) ?? 0, item.cooldown.resetAt));
		candidatesToConsider.sort(
			(a, b) => readyAt.get(a.index)! - readyAt.get(b.index)! || b.cooldown.resetAt - a.cooldown.resetAt,
		);
		...
		this.emit({
			...
			until: readyAt.get(earliest.index)!,
	...
	async prompt(prompt: string, maxTurns = 100, budget?: RunDeadline): Promise<AssistantMessage> {
		...
		while (true) {
			...
			const answerLimits =
				!thrown && final && !emptyAnswer && final.stopReason !== "error" && final.stopReason !== "aborted"
					? observeAnswer(
							candidate,
							final.content.flatMap((block) => (block.type === "text" ? [block.text] : [])).join("\n"),
							final.usage?.output ?? -1,
			...
			if (
				...
				const learned = responses.flatMap((response) =>
					observeLimits(candidate, response, policy, (this.options.now ?? Date.now)()),
				);
			...
			const failure: FailureClassification = emptyAnswer
				...
				: answerLimits.length > 0
					? {
							...
							errorClass: "AnswerLimit",
							resetAt: Math.max(...answerLimits.map((entry) => entry.resetAt)),
						}
					: classifyFailure(
							{ error: thrown, message: final, response, now: (this.options.now ?? Date.now)() },
							config?.quota ? { rules: config.quota.rules, model: candidate.model } : undefined,
						);
			const stopReason = final?.stopReason ?? "threw";
			const errorMessage = emptyAnswer
			...
			await this.recordOutcome(
				...
				false,
				final?.usage,
				(this.options.now ?? Date.now)() - attemptStartedAt,
				failure.kind,
			);
			...
			const observed = observeLimits(
				candidate,
				{ status: response?.status ?? failure.status ?? 0, headers: response?.headers, body: errorMessage },
				this.policy(candidate),
				observedAt,
			).map((entry) =>
				failure.resetAt === undefined || entry.resetAt >= failure.resetAt
					? entry
					: { ...entry, resetAt: failure.resetAt, source: "rule" as const },
			);
			const dailyWording = /per[- ]?day|perday|\bdaily\b/iu.test(errorMessage);
			const type =
				(failure.kind === "rate_limited" || failure.kind === "quota_exhausted") && dailyWording
					? "daily"
					: failure.kind === "rate_limited"
						? "rate"
						: failure.kind === "quota_exhausted"
							? /monthly|billing cycle/iu.test(errorMessage)
								? "monthly"
								: /resets? in|weekly|\b5h/iu.test(errorMessage)
									? "window"
									: "daily"
						...
						: type
							? [
									defaultLimit(
										...
										type === "daily"
											? failure.resetAt
											: (failure.resetAt ?? (failure.kind === "rate_limited" ? observedAt + 60_000 : undefined)),
										undefined,
										failure.pool,
										this.policy(candidate),
									),
			...
			if (
				...
				(leaf.message.stopReason === "error" ||
					...
					isEmptyAnswer(leaf.message) ||
					(answerLimits.length > 0 && leaf.message === final))
			) {
...
export async function createFailoverSupervisor(options: CreateSupervisorOptions): Promise<FailoverSupervisor> {
	...
	const ledger = new LimitLedger(home, {
		fallbackTtlMs: options.cooldownTtlMs,
		persist: (candidate) => !ephemeral.has(candidate.provider),
	});
	let activeIndex = -1;
	...
	for (let index = 0; index < options.chain.length; index++) {
		...
		for (const entry of await ledger.recover((options.now ?? Date.now)()))
			options.onEvent?.({ type: "recovered", entry });
		const config = options.providerConfigs?.get(candidate.provider);
		const pools = (config?.limits?.defaults ?? []).flatMap((entry) =>
			entry.pool ? [entry.pool.replace(":model", `:${candidate.model}`)] : [],
		);
		...
		if (options.quota) {
			...
			if (admission.decision !== "admit") {
				...
				const kind: FailureKind = admission.entries.some((entry) => entry.type === "daily" || entry.type === "monthly")
					? "quota_exhausted"
					: "rate_limited";
				...
				options.onEvent?.({
					...
					candidate,
					reason: kind,
					...(admission.waitUntil === undefined ? {} : { resetAt: admission.waitUntil }),
				});
				continue;
			}
		}
		...
		for (const item of config?.limits?.defaults ?? []) {
			...
			entries.push({
				...
				resetAt: currentNow + item.windowMs,
				source: "default",
				remaining: item.limit,
				limit: item.limit,
			});
		...
		const model = config?.models.static.find((entry) => entry.id === candidate.model);
		const verdict = options.taskEstimate
			? assessCandidate(candidate, options.taskEstimate, entries, {
					contextWindow: model?.contextWindow,
					tier: model?.tier,
					reserve: config?.limits?.reserve,
				})
			: { eligible: (await ledger.blocking(candidate, currentNow, config?.limits?.reserve, pools)).length === 0 };
		if (verdict.eligible) {
			activeIndex = index;
			break;
		}
		const cooldown = persistedEntries[0];
		const kind: FailureKind = !cooldown
			? "rate_limited"
			: cooldown.type === "auth"
				? "auth"
				: cooldown.type === "overload"
					? "transient"
					: cooldown.type === "rate"
						? "rate_limited"
						: "quota_exhausted";
		...
		initialReasons.push({
			candidate,
			kind,
			message: cooldown ? `Candidate is limited (${cooldown.type})` : "Candidate cannot fit one task step",
		});
		options.onEvent?.({
			type: "candidate_skipped",
			candidate,
			reason: kind,
			...(cooldown ? { resetAt: cooldown.resetAt } : {}),
		});
	}
	if (activeIndex < 0) {
		...
		for (const entry of limits.filter(
			(item) =>
				item.resetAt > nowAt &&
				options.chain.some(
					(candidate) =>
		...
		if (earliest && waitMs <= (options.maxWaitMs ?? 5 * 60_000)) {
			...
			for (const entry of await ledger.recover((options.now ?? Date.now)()))
				options.onEvent?.({ type: "recovered", entry });
			activeIndex = options.chain.findIndex(
				(candidate) =>
					candidate.provider === earliest.provider &&
					candidate.account === earliest.account &&
					candidate.model === earliest.model,
			);
		}
