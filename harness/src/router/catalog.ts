...
function capabilityFromCatalog(
	...
	catalog: CatalogModel & { contextWindow?: number; tier?: ModelCapability["limitTier"] },
	account: string,
	...
	const override: ModelCapabilityOverride =
		overrides?.[key(provider.id, catalog.id, account)] ?? overrides?.[key(provider.id, catalog.id)] ?? {};
	...
	const modalities: ModelModality[] = [
		...new Set<ModelModality>([
			...inputs,
			...(/image/iu.test(methods) ? ["image_gen" as const] : []),
			...(/video/iu.test(methods) ? ["video_gen" as const] : []),
		]),
	];
	return {
		...
		modalities,
		quality: {},
		limitTier: declared?.tier ?? catalog.tier ?? "standard",
...
export function getUsableModels(
	...
	context: { modelPoller: Poller; provider: ProviderConfig; overrides?: RouterConfig["models"] },
...
export function buildRouterCatalog(
	options: BuildOptions | BuildOptionsWithPoller,
	...
	for (const provider of options.providers) {
		...
		const catalogModels =
			live?.models ?? provider.models.static.map((model) => ({ id: model.id, name: model.name ?? model.id }));
		const accounts = options.accounts?.get(provider.id) ?? ["default"];
		for (const account of accounts)
			for (const catalog of catalogModels)
				result.push(capabilityFromCatalog(provider, catalog, account, live?.source ?? "builtin", options.overrides));
	}
	return result;
}
...
async function buildRouterCatalogFromPoller(options: BuildOptionsWithPoller): Promise<ModelCapability[]> {
	...
	for (const provider of options.providers) {
		...
		for (const account of accounts)
			result.push(
				...(await getUsableModels(provider.id, account, {
					modelPoller: options.modelPoller,
					provider,
