...
The manuscript structure and content are derived exclusively from the concept folder tree.
...
- Concept files own terminology and all manuscript content. `concepts/manuscript/` owns document-level material such as the introduction, goals, research questions, methodology, results/discussion, conclusion, and appendices; domain concepts own theory/practical projections.
- `concepts/manuscript/appendices/` is the canonical appendix root. Appendix entries are represented by child folders so their section concepts render as top-level appendix headings.
- Typst imports remain explicit because source directories cannot be enumerated dynamically at runtime.
- Substantive manuscript prose outside `concepts/**` is prohibited.
